...
Page({
  ...
  clickbrg: function (event){
    ...
    // var portid = event.currentTarget.dataset.name;
    this.setData({
      portid: name
    })
    this.setData({
  ...
  binviewbut: function (e) {
    var that = this;
    var deviceid = e.currentTarget.dataset.device; //设备Id
        ...
        success: res => {
         console.log(res)
          if (res.data.code == 200) {
            ...
            } else if (res.data.result.balance !== 'ok'){
              wx.showModal({
                title: '提示',
                content: '余额不足',
                confirmText: '去充值',
  ...
  onLoad: function (options) {
    ...
    //分钟计算
    wx.request({
      url: app.globalData.URL + '/DevicePowerPricing/findList',
      data: {
        token: token,
        deviceId: deviceid
      },
